...
const routedComponents = [_survey_survey_component__WEBPACK_IMPORTED_MODULE_3__["SurveyComponent"]];
...
class Student {
    constructor(StudentId, firstname, lastname, address, city, state, zip, phone, email, homePage, dateprovided, likeCampus, interest, comments, graduateMonth, graduateYear, mean, recommend) {
...
let SurveyComponent = class SurveyComponent {
    ...
    onSubmit(student) {
        ...
        const s = Math.sqrt(array.map(x => Math.pow(x - avg, 2)).reduce((a, b) => a + b) / array.length);
    ...
    getStudents(url) {
        ...
        this.http.get("https://6e40k1nt0f.execute-api.us-east-1.amazonaws.com/prod/student")
            ...
            this.students.surveys.forEach(studentInArray => {
                const personObj = {};
                Object.keys(studentInArray).forEach(keyOfObject => {
                    const value = studentInArray[keyOfObject];
